...
RAW_RNA_FILE = "thymus.h5ad"
BASE_DATASET_FILE = "Thymus_base.h5ad"
DEFAULT_N_SUPER_CELLS = 120


MIN_TOTAL_COUNTS = 2000
MAX_TOTAL_COUNTS = 30000
MIN_GENES_BY_COUNTS = 800
...
def add_cell_type_annotations(s):
    if "cell_type" not in s.obs:
        raise ValueError('Missing Thymus annotation column: "cell_type"')

    s.obs["cell_type"] = s.obs["cell_type"].to_numpy()
    if "Sample" in s.obs:
        s.obs["sample"] = s.obs["Sample"].to_numpy()
    if "donor" in s.obs:
        s.obs["batch"] = s.obs["donor"].to_numpy()
    elif "Sample" in s.obs:
        s.obs["batch"] = s.obs["Sample"].to_numpy()
    return s
