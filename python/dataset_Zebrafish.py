...
RAW_RNA_FILE = "zf_atlas_bud_to_10dpf.h5ad"
BASE_DATASET_FILE = "Zebrafish_base.h5ad"
DEFAULT_N_SUPER_CELLS = 500


MIN_TOTAL_COUNTS = 1000
#MAX_TOTAL_COUNTS = 125000
# fixed long tail of library sizes
MAX_TOTAL_COUNTS = 20000
MIN_GENES_BY_COUNTS = 500
#MAX_GENES_BY_COUNTS = 12000
MAX_GENES_BY_COUNTS = 6000
MAX_PCT_COUNTS_MT = 10
MIN_GENE_COUNTS = 50


def compute_qc_metrics(s):
    X = s.X
    ...
    if "mt" in s.var:
        mt_gene = np.asarray(s.var["mt"]).astype(bool)
    else:
        mt_gene = np.asarray(s.var_names.str.startswith("mt-"))
...
def add_cell_type_annotations(s):
    annotation_column = "zebrafish_anatomy_ontology_class"
    if annotation_column not in s.obs:
        raise ValueError(f"Missing Zebrafish annotation column: {annotation_column}")

    s.obs["cell_type"] = s.obs[annotation_column].to_numpy()
    s.obs["sample"] = s.obs["fish"].to_numpy()
    s.obs["batch"] = s.obs["fish"].to_numpy()
    return s
...
def load_base_dataset(base_f, overwrite=False, out_f=None):
    ...
    if os.path.isfile(out_f) and not overwrite:
        ...
        if not all(column in s.obs for column in ("cell_type", "sample", "batch")):
    ...
    s.var = s_raw.var.copy()
    s.uns = s_raw.uns.copy()
    if "X_umap" in s_raw.obsm:
        s.obsm["X_umap"] = s_raw.obsm["X_umap"].copy()
